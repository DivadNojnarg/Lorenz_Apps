...
void RxODE_mod_lorenz_RxODE_dydt(unsigned int neq, double t, double *A, double *DADT);
void RxODE_mod_lorenz_RxODE_calc_lhs(double t, double *A, double *lhs);

//--------------------------------------------------------------------------
void RxODE_mod_lorenz_RxODE_dydt_lsoda_dum(int *neq, double *t, double *A, double *DADT)
{
	RxODE_mod_lorenz_RxODE_dydt(*neq, *t, A, DADT);
}
...
void call_lsoda(int neq, double *x, int *evid, int nx, double *inits, double *dose, double *ret, int *rc)
{
	...
	for(i=0; i<nx; i++)
	{
		...
		{
	        F77_CALL(dlsoda)(RxODE_mod_lorenz_RxODE_dydt_lsoda_dum, &neq, yp, &xp, &xout, &itol, &rtol, &atol, &itask,
...
void RxODE_mod_lorenz_RxODE_dydt_dvode_dum(int *neq, double *t, double *A, double *DADT, double *RPAR, int *IPAR)
{
	RxODE_mod_lorenz_RxODE_dydt(*neq, *t, A, DADT);
}
...
void call_dvode(int neq, double *x, int *evid, int nx, double *inits, double *dose, double *ret, int *rc)
{
	...
	for(i=0; i<nx; i++)
	{
		...
		if(xout-xp > DBL_EPSILON*max(fabs(xout), fabs(xp)))
		{
	        F77_CALL(dvode)(RxODE_mod_lorenz_RxODE_dydt_dvode_dum, &neq, yp, &xp, &xout, &itol, &rtol, &atol, &itask,
	...
	for(i=0; i<nx; i++)
	{
		...
		if(xout-xp > DBL_EPSILON*max(fabs(xout), fabs(xp)))
		{
			idid = dop853(
							  ...
							  RxODE_mod_lorenz_RxODE_dydt,       	/* function computing the value of f(x,y) */
							  xp,           /* initial x-value */
...
//wrapper
void RxODE_mod_lorenz_RxODE_ode_solver(
	int *neq,
	double *theta,	//order:
	double *time,
	int *evid,
...
)
{
	...
	if (*nlhs) for (i=0; i<*ntime; i++)
		RxODE_mod_lorenz_RxODE_calc_lhs(time[i], ret+i*(*neq), lhs+i*(*nlhs));
