...
extern void lorenz_RxODE__ode_solver(int *neq,
  ...
  //if (_ptrid() != 1536649797 ){ _assign_ptr(lorenz_RxODE__model_vars());}
  double *_theta = theta;
...
void lorenz_RxODE__dydt(int *_neq, double t, double *__zzStateVar__, double *__DDtStateVar__)
{
  ...
  __DDtStateVar__[0]=_IR[0]+k1*(x2-x1);
  __DDtStateVar__[1]=_IR[1]+k2*x1-x2-x1*x3;
  __DDtStateVar__[2]=_IR[2]+k3*x3+x1*x2;
...
void lorenz_RxODE__calc_lhs(int _cSub, double t, double *__zzStateVar__, double *_lhs) {
  ...
  __DDtStateVar_0__=_IR[0]+k1*(x2-x1);
  __DDtStateVar_1__=_IR[1]+k2*x1-x2-x1*x3;
  __DDtStateVar_2__=_IR[2]+k3*x3+x1*x2;
...
extern SEXP lorenz_RxODE__model_vars(){
  ...
  if (!_rxIsCurrentC(_mv)){
    ...
    SET_STRING_ELT(params,0,mkChar("k1"));
    SET_STRING_ELT(params,1,mkChar("k2"));
    SET_STRING_ELT(params,2,mkChar("k3"));
    SET_STRING_ELT(state,0,mkChar("x1"));
    SET_STRING_ELT(normState,0,mkChar("x1"));
    _SR[0] = 0;
    SET_STRING_ELT(state,1,mkChar("x2"));
    SET_STRING_ELT(normState,1,mkChar("x2"));
    _SR[1] = 0;
    SET_STRING_ELT(state,2,mkChar("x3"));
    SET_STRING_ELT(normState,2,mkChar("x3"));
    _SR[2] = 0;
    ...
    SET_STRING_ELT(model,1,mkChar("d/dt(x1)=k1*(x2-x1);\nd/dt(x2)=k2*x1-x2-x1*x3;\nd/dt(x3)=k3*x3+x1*x2;\n"));
    SET_STRING_ELT(modeln,2,mkChar("parseModel"));
    SET_STRING_ELT(model,2,mkChar("__DDtStateVar__[0] = _IR[0] + k1*(x2-x1);\n__DDtStateVar__[1] = _IR[1] + k2*x1-x2-x1*x3;\n__DDtStateVar__[2] = _IR[2] + k3*x3+x1*x2;\n"));
    ...
    SET_STRING_ELT(mmd5,0,mkChar("5b5f4b7e8c2f5112a27e1b294653a1b7"));
    SET_STRING_ELT(mmd5n,1,mkChar("parsed_md5"));
    SET_STRING_ELT(mmd5,1,mkChar("9f3b34f85ad2056cb4a731aa87009600"));
